      ...
      if (storedAuth) {
        ...
        handleFetch(async () => {
          ...
          if (response.status === 401)
            throw new UnAuthenticatedException("Please login again.", response.status);

          if (response.status === 403)
            throw new UnAuthorizedException("You are not authorized to access the resources.", response.status);
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 pt-3 border-t border-slate-50">
                  <div className="flex items-center gap-6 text-xs text-slate-500 font-bold">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span>Duration: <span className="text-slate-800 font-black">{item.examDuration}</span></span>
                    </div>
