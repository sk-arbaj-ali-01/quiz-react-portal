...
export default function StudentQuizView() {
    ...
    useEffect(()=>{
        ...
        .then(res => res.json())
        .then(data => {
            setGroupId(groupIdFromQuery || data?.groupId || '');
    ...
    const submitQuizAnswers = async () => {
        ...
        questions.forEach((question) => {
            ...
            preparedQuestions.push({
                questionId: question.id,
                questionType: question.type,
                optionId: question.type === 'MCQ' ? (question.selectedOption ?? null) : null,
            });
        });
        ...
        try {
            ...
            const response = await fetch('https://localhost:7087/v1/questions/submit-answers', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'x-user-id': USER_ID,
                },
                body: JSON.stringify({ requestDto: payload })
            });
